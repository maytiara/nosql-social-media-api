...
module.exports = {
	...
	updateUser(req, res) {
		User.findOneAndUpdate(
			...
			{ runValidators: true, new: true } // it updated the new insert value
		).then((users) => {
			res.json(users);
		});
	},
	...
	// remove user by _id
	deleteUser(req, res) {
		User.findOneAndDelete({ users: req.body.usersId }) //filter
			.then((users) => {
				if (!users) {
	...
	createFriend(req, res) {
		User.findOneAndUpdate(
			...
			{ $addToSet: { path: "friends", $push: { friends: req.body } } },
